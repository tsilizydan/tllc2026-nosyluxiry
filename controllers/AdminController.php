...
class AdminController extends Controller
{
    ...
    {
        $bookingModel = new Booking();
        $result = $bookingModel->paginate($this->getPage(), 20, '1', [], 'created_at DESC');
    ...
    public function updateBookingStatus(): void
    {
        ...
        $db = Database::getInstance();
        $db->update('bookings', ['status' => $status], 'id = ?', [(int)$this->input('id')]);
    ...
    public function approveReview(): void
    {
        if (!$this->validateCsrf()) { $this->json(['error' => 'Invalid token'], 403); return; }
        $db = Database::getInstance();
        $db->update('reviews', ['is_approved' => 1], 'id = ?', [(int)$this->input('id')]);
