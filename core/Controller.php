...
class Controller
{
    ...
    protected function view(string $view, array $data = [], string $layout = 'public'): void
    {
        ...
        if (!file_exists($viewFile)) {
            $this->errorPage(500, 'View not found');
            return;
        }
        include $viewFile;
    ...
    protected function requireAdmin(): void
    {
        $this->requireAuth();
        if (!Auth::isAdmin()) {
            $this->errorPage(403, 'Access denied');
        }
    }
     ...
     * Render error page
     */
    protected function errorPage(int $code, string $message = ''): void
    {
        http_response_code($code);
        $errorFile = VIEWS_PATH . '/errors/' . $code . '.php';
        if (file_exists($errorFile)) {
            $this->view('errors.' . $code, ['pageTitle' => $this->setTitle($code . ' Error')]);
        } else {
            // Fallback for uncovered error codes
