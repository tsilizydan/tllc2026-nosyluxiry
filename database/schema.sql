...
CREATE TABLE IF NOT EXISTS `partners` (
    `id` INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    `type` ENUM('hotel','car_rental','currency_exchange') NOT NULL,
    `name` VARCHAR(255) NOT NULL,
    `slug` VARCHAR(255) NOT NULL UNIQUE,
    `description` TEXT DEFAULT NULL,
    `short_description` VARCHAR(500) DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `payment_methods` (
    ...
    `icon` VARCHAR(100) DEFAULT 'fas fa-credit-card',
...
-- Partners seed data
INSERT INTO `partners` (`type`, `name`, `slug`, `description`, `short_description`, `location`, `contact_email`, `contact_phone`, `website`, `is_featured`, `status`, `sort_order`) VALUES
('hotel', 'Anjajavy Le Lodge', 'anjajavy-le-lodge', 'A stunning luxury eco-lodge nestled in a private nature reserve on Madagascar''s remote northwest coast. Accessible only by air, Anjajavy offers an exclusive escape with world-class cuisine, private beach, and exceptional wildlife encounters.', 'Luxury eco-lodge in a private nature reserve.', 'Anjajavy, Northwestern Madagascar', 'reservations@anjajavy.com', '+261 20 22 490 69', 'https://anjajavy.com', 1, 'active', 1),
('hotel', 'Constance Tsarabanjina', 'constance-tsarabanjina', 'An idyllic private island resort in the Mitsio archipelago, offering barefoot luxury with just 25 villas set on pristine white sand. Perfect for honeymoons and romantic getaways.', 'Private island resort with barefoot luxury.', 'Mitsio Archipelago, Nosy Be', 'info@tsarabanjina.com', '+261 20 22 394 55', 'https://constancehotels.com', 1, 'active', 2),
('car_rental', 'Madagascar Car Hire', 'madagascar-car-hire', 'Reliable 4x4 vehicle rental service with experienced drivers who know every road in Madagascar. Fleet includes Toyota Land Cruisers, Hilux, and comfortable minibuses for group travel.', 'Reliable 4x4 rentals with experienced drivers.', 'Antananarivo, Madagascar', 'booking@mdgcarhire.com', '+261 34 12 345 67', 'https://madagascarcarhire.com', 1, 'active', 3);

-- Payment methods seed data
INSERT INTO `payment_methods` (`code`, `name`, `description`, `icon`, `is_active`, `sort_order`) VALUES
('stripe', 'Credit / Debit Card', 'Pay securely with Visa, Mastercard, or American Express via Stripe.', 'fas fa-credit-card', 0, 1),
('paypal', 'PayPal', 'Pay with your PayPal account or debit/credit card.', 'fab fa-paypal', 0, 2),
('bank_transfer', 'Bank Transfer', 'Transfer directly to our bank account. Booking confirmed upon receipt.', 'fas fa-university', 1, 3),
('wise', 'Wise (TransferWise)', 'International transfer with low fees via Wise.', 'fas fa-exchange-alt', 0, 4),
('cash', 'Cash Payment', 'Pay in cash upon arrival. Advance deposit may be required.', 'fas fa-money-bill-wave', 1, 5);
